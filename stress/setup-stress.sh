...
_count_enforced() {
  ...
  _out=$(oc get "$KIND" -n "$NS_TARGET" -l stress-test=true \
    ...
    2>/dev/null) || true
  # grep -c already prints "0" on zero matches; suppress its exit-1 with || true
  # so the subshell doesn't add a second "0" via the fallback.
  echo "$_out" | grep -c "Enforced=True" 2>/dev/null || true
}

...
```bash
# a) Base PoC must be fully deployed first
oc apply -f mtls-apikey/00-namespace.yaml
# ... (follow mtls-apikey/README.md setup steps)

# b) Scale Authorino BEFORE the test (stateless — each replica handles auth independently)
...
# c) Increase kuadrant-operator-controller-manager memory limits (https://issues.redhat.com/browse/CONNLINK-678)
...
#    Patch the CSV (spec.install.spec.deployments[0].spec.template.spec.containers[0].resources)
#    — OLM reads this manifest and applies it; the change survives operator reconciliation.
...
# Generate CA + base certs (if not already done)
chmod +x mtls-apikey/02-generate-client-certs.sh
./mtls-apikey/02-generate-client-certs.sh

# Generate shared stress cert (auto-done by setup-stress.sh if missing)
chmod +x mtls-apikey/stress/gen-stress-certs.sh
